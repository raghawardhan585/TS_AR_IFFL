...
def Ts_Ar_Iffl(x,t, couple_ts_ar = 1.,couple_ar_iffl = 1.):
    ...
    xdot[2] = u_ar - ar_gamma_A * x[2] + ar_kappa_A / ar_delta_A * (ar_alpha_A * (x[2] / ar_K_A) ** ar_n + ar_alpha_A0) / (1 + (x[2] / ar_K_A) ** ar_n + (x[3] / ar_K_B) ** ar_m)
    xdot[3] = - ar_gamma_B * x[3] + ar_kappa_B / ar_delta_B * (ar_alpha_B * (x[2] / ar_K_A) ** ar_n + ar_alpha_B0) / (1 + (x[2] / ar_K_A) ** ar_n)
    # Incoherent feed forward loop
    u_iffl = u_iffl_step + couple_ar_iffl * x[3]
    xdot[4] = u_iffl - iffl_gamma1 * x[4]
    xdot[5] = u_iffl - iffl_gamma3 * (x[4] / iffl_Kd)**n_iffl / (1 + (x[4] / iffl_Kd)**n_iffl) - iffl_gamma2 * x[5]
    # xdot[4] = iffl_k0 * u_iffl/(0.8 + u_iffl)**2 - iffl_gamma1 * x[4]
    # xdot[5] = iffl_k1 * u_iffl / (1 + (x[4] / iffl_Kd) + (u_iffl / 0.8)) - iffl_gamma2 * x[5]
    return xdot
...
def Ar_Iffl_Ts(x,t, couple_ar_iffl = 1.,couple_ar_ts = 1.):
    xdot = np.zeros(len(x))
    # Activator Repressor
    ...
    xdot[2] = u_iffl - iffl_gamma1 * x[2]
    xdot[3] = u_iffl - iffl_gamma3 * (x[2] / iffl_Kd)**n_iffl / (1 + (x[2] / iffl_Kd)**n_iffl) - iffl_gamma2 * x[3]
    # Toggle Switch
    u_ts = u_ts_step + couple_ar_ts * x[1]
    xdot[4] = ts_beta / (1 + (x[5] / ts_K) ** ts_n) - ts_gamma * x[4] + u_ts
    xdot[5] = ts_beta / (1 + (x[4] / ts_K) ** ts_n) - ts_gamma * x[5]
    return xdot
def Ar_Iffl(x,t, couple_ar_iffl = 1.,couple_ar_ts = 1.):
    xdot = np.zeros(len(x))
    # Activator Repressor
    xdot[0] = - ar_gamma_A * x[0] + ar_kappa_A / ar_delta_A * (ar_alpha_A * (x[0] / ar_K_A) ** ar_n + ar_alpha_A0) / (1 + (x[0] / ar_K_A) ** ar_n + (x[1] / ar_K_B) ** ar_m)
    xdot[1] = - ar_gamma_B * x[1] + ar_kappa_B / ar_delta_B * (ar_alpha_B * (x[0] / ar_K_A) ** ar_n + ar_alpha_B0) / (1 + (x[0] / ar_K_A) ** ar_n)
    ...
    xdot[3] = u_iffl - iffl_gamma3 * (x[2] / iffl_Kd) ** n_iffl / (1 + (x[2] / iffl_Kd) ** n_iffl) - iffl_gamma2 * x[3]
    return xdot
def Ar_Ts(x,t, couple_ar_iffl = 1.,couple_ar_ts = 1.):
    xdot = np.zeros(len(x))
    # Activator Repressor
    xdot[0] = - ar_gamma_A * x[0] + ar_kappa_A / ar_delta_A * (ar_alpha_A * (x[0] / ar_K_A) ** ar_n + ar_alpha_A0) / (1 + (x[0] / ar_K_A) ** ar_n + (x[1] / ar_K_B) ** ar_m)
    xdot[1] = - ar_gamma_B * x[1] + ar_kappa_B / ar_delta_B * (ar_alpha_B * (x[0] / ar_K_A) ** ar_n + ar_alpha_B0) / (1 + (x[0] / ar_K_A) ** ar_n)
    # Toggle Switch
    u_ts = u_ts_step + couple_ar_ts * x[1]
    xdot[2] = ts_beta / (1 + (x[3] / ts_K) ** ts_n) - ts_gamma * x[2] + u_ts
    xdot[3] = ts_beta / (1 + (x[2] / ts_K) ** ts_n) - ts_gamma * x[3]
    return xdot
def Ar(x,t, couple_ar_iffl = 1.,couple_ar_ts = 1.):
    xdot = np.zeros(len(x))
    # Activator Repressor
    xdot[0] = - ar_gamma_A * x[0] + ar_kappa_A / ar_delta_A * (ar_alpha_A * (x[0] / ar_K_A) ** ar_n + ar_alpha_A0) / (1 + (x[0] / ar_K_A) ** ar_n + (x[1] / ar_K_B) ** ar_m)
    xdot[1] = - ar_gamma_B * x[1] + ar_kappa_B / ar_delta_B * (ar_alpha_B * (x[0] / ar_K_A) ** ar_n + ar_alpha_B0) / (1 + (x[0] / ar_K_A) ** ar_n)
...
dict_output_params['Iffl'] = {'act_state_no':2, 'rep_state_no':3, 'Vm':1, 'KA':0.02, 'KB':1, 'nA':1, 'nB':2}
dict_output_params['Ts'] = {'act_state_no':4, 'rep_state_no':5, 'Vm':1, 'KA':120, 'KB':1, 'nA':1, 'nB':3}
x0_init = np.array([])
x0_init = np.concatenate([x0_init, x0_init_ar])
if iffl_active:
    x0_init = np.concatenate([x0_init, x0_init_iffl])
    if toggle_switch_active:
        x0_init = np.concatenate([x0_init, x0_init_ts])
        simulation_system = Ar_Iffl_Ts
    else:
        simulation_system = Ar_Iffl
else:
    if toggle_switch_active:
        x0_init = np.concatenate([x0_init, x0_init_ts])
        simulation_system = Ar_Ts
        dict_output_params['Ts']['act_state_no'] = 2
        dict_output_params['Ts']['rep_state_no'] = 3
    else:
        simulation_system = Ar
...
for i in range(N_SIMULATIONS):
    ...
    X = odeint(simulation_system,x0_init_i,t,(couple_Ar_Iffl,couple_Ar_Ts))
    # Make the Output
    ...
    if iffl_active:
        ...
        Y = np.concatenate([Y, output_type_1(X[:, [dict_output_params['Iffl']['act_state_no']]],
                                             X[:, [dict_output_params['Iffl']['act_state_no']]],
                                             Vm=dict_output_params['Iffl']['Vm'], KA=dict_output_params['Iffl']['KA'],
                                             KB=dict_output_params['Iffl']['KB'], nA=dict_output_params['Iffl']['nA'],
                                             nB=dict_output_params['Iffl']['nB'])], axis=1)
    if toggle_switch_active:
        # Y = np.concatenate([Y, output_type_1(X[:, 4:5], X[:, 5:6], Vm=1,KA=0.75,KB=1000,nA=1,nB=1)], axis=1)
        # Y = np.concatenate([Y, output_type_1(X[:, [dict_output_params['Ts']['act_state_no']]],
        #                                      X[:, [dict_output_params['Ts']['act_state_no']]],
...
save_system(ls_data, dict_measurement_info = dict_measurement_info_6, SYSTEM_NO =4)
